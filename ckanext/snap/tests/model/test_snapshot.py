...
class TestSnapshot:
    def test_by_hello(self, faker: Faker, something_factory: Any):
        ...
        smth = model.Session.scalar(Snapshot.by_hello(first["hello"]))
        assert smth.id == first["id"]

        smth = model.Session.scalar(Snapshot.by_hello(second["hello"]))
    ...
    def test_dictization(self, something: dict[str, Any]):
        ...
        smth = model.Session.get(Snapshot, something["id"])
        assert smth.dictize({}) == {
            "id": something["id"],
