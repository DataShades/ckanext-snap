...
def test_page_redirect(app: CKANTestApp):
    ...
    expected = tk.url_for("snap.page", _external=True)
...
class TestComplex:
    def test_get(self, app: CKANTestApp, faker: Faker):
        ...
        word = faker.word()
        resp = app.get(tk.url_for("snap.complex", word=word))
    ...
    def test_post_invalid(self, app: CKANTestApp, faker: Faker):
        ...
        word = faker.word()
        resp = app.post(tk.url_for("snap.complex", word=word))
        page = bs4.BeautifulSoup(resp.body)
    ...
    def test_post_valid(self, app: CKANTestApp, faker: Faker):
        ...
        word = faker.word()
        resp = app.post(
            tk.url_for("snap.complex", word=word),
            data={"left": "1", "right": "2"},
        )
        page = bs4.BeautifulSoup(resp.body)
