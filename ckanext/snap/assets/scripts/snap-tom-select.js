 ...
 */
ckan.module("snap-tom-select", function () {
  ...
  return {
    ...
    // `this.sandbox.snap.nestedOptions` and we receive
    // `{helloWorld: {byeWorld: ...}}`.
    options: {
    ...
    initialize() {
      ...
      if (typeof TomSelect === "undefined") {
        ...
        console.error("[snap-tom-select] TomSelect library is not loaded");
        return
      }

      // tom-select has a number of nested options. We are using
      // `nestedOptions` helper defined inside `snap.js` to
      // convert flat options of CKAN JS module into nested object.
      const options = this.sandbox["snap"].nestedOptions(this.options);
